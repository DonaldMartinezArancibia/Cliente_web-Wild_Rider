 ...
 * Selector de idioma.
 *
 * Resuelve la traducción de la página actual a partir del `remoteTypeName` del
 * contexto: una sola query, una sola función de resolución y un `map` sobre
 * los idiomas configurados, en lugar de un bloque por content type y otro por
 * idioma.
 */
export default function LanguageSelector({ pageContext, langSelectorTitle }) {
  ...
  // aún no ha elegido uno, se le lleva a su idioma.
  useEffect(() => {
    const browserLanguage = navigator.language.split("-")[0]
    if (getRememberedLanguage()) return
    if (!AUTO_REDIRECT_LANGUAGES.includes(browserLanguage)) return
    if (browserLanguage === langKey) return

    navigate(localePath(browserLanguage))
  }, [langKey])

  if (loading || error) return null

  const goToLanguage = locale => {
    rememberLanguage(locale)
    navigate(translatedPath(data, locale, isHome))
  }
    ...
    <div className="relative inline-block m-[auto_20px_auto_auto] min-[800px]:m-[auto_60px_auto_auto] lg:m-auto">
      ...
      <ul className="font-black text-center text-white transition-shadow left-1/4 hover:shadow-lg">
        {SELECTABLE_LANGUAGES.filter(
          language =>
            language.code !== langKey &&
            langSelectorTitle?.[language.displayField] !== false
        ).map(language => (
          <li key={language.code}>
            <a
              ...
              onClick={event => {
                ...
                goToLanguage(language.code)
              }}
              title={langSelectorTitle?.[language.tooltipField]}
            >
              {langSelectorTitle?.[language.titleField]}
            </a>
          </li>
        ))}
      </ul>
